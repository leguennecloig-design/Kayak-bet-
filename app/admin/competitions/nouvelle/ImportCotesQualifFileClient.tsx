...
type ParsedQualifCategory = {
  code: string;
  libelle: string;
  nb_partants: number;
  nb_qualifies: number;
  athletes: ParsedQualifAthlete[];
};
...
type ParseResult = {
  ...
  categories: ParsedQualifCategory[];
  stats: { total: number; categories: number };
  errors: string[];
};

export default function ImportCotesQualifFileClient({ onBack }: { onBack: () => void }) {
  ...
  async function handleFile(file: File) {
    ...
    fd.append("file", file);
    try {
      const res = await fetch("/api/admin/parse-cotes-qualif-file", { method: "POST", body: fd });
      if (!res.ok) {
  ...
  async function handleImport() {
    ...
    setError(null);
    try {
      const res = await fetch("/api/admin/import-cotes-qualif-file", {
        method: "POST",
    ...
    <div className="max-w-2xl">
      <div className="mb-8">
        ...
        <h1 className="font-anton italic uppercase text-white text-[36px] leading-[0.9] mt-4">
          Compétition<br />
          <span className="text-cyan">qualif → finale</span>
        </h1>
        <p className="text-[#7c9aaa] font-archivo text-[13px] mt-3 leading-relaxed">
          Importe un fichier .txt contenant déjà la liste de départ ET les cotes de
          passage en finale calculées en externe. Un seul marché existe pour cette
          compétition : la probabilité de se qualifier pour la finale — pas de
      ...
      {!result && !parsing && (
        ...
        >
          ...
          <p className="font-grotesk font-semibold text-white text-[15px] mb-1.5">
            Glisser-déposer le fichier .txt de cotes qualif
          </p>
            ...
            <div className="divide-y divide-[var(--border)]">
              {result.categories.map((cat) => (
                <div key={cat.code} className="flex items-center gap-3 px-4 py-2.5 hover:bg-white/[.02]">
                  ...
                  <span className="text-[11px] text-[#7c9aaa] font-mono w-32 text-right">
                    {cat.athletes.length} dép. · {cat.nb_qualifies} qualif.
                  </span>
                </div>
              ))}
            </div>
