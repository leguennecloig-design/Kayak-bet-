...
export default function NotificationBell({ onFriendsChanged }: { onFriendsChanged?: () => void }) {
  ...
  return (
    <>
      ...
      {/* Portail vers document.body : ce composant est rendu dans <header> qui a
          un backdrop-filter, or filter/backdrop-filter sur un ancêtre crée un
          nouveau containing block pour position:fixed — sans portail, ce
          scrim "fixed" restait cadré sur les ~70px du header au lieu de tout
          l'écran (le panneau apparaissait à plat dans la page, infermable).
          Le wrapper .kb-app est nécessaire : tout le CSS de la modale est
          scopé ".kb-app .kb-modal..." et le portail sort du vrai .kb-app. */}
      {open && createPortal(
