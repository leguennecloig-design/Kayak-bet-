// Port du composant React Bits <Ferrofluid /> — rend un blob WebGL
// "ferrofluide" dans un conteneur DOM via ogl.
// Usage : const handle = mountFerrofluid(containerEl, { colors: [...], speed: .4, ... });
//         handle.destroy() pour démonter.
...
export function mountFerrofluid(container: HTMLElement, opts: FerrofluidOptions = {}) {
  const {
    dpr,
    paused = false,
  ...
  return {
    ...
    destroy() {
      ...
      const p = program as unknown as { remove?: () => void };
      const g = geometry as unknown as { remove?: () => void };
      const m = mesh as unknown as { remove?: () => void };
      const r = renderer as unknown as { destroy?: () => void };
      p.remove?.();
      g.remove?.();
      m.remove?.();
      r.destroy?.();
